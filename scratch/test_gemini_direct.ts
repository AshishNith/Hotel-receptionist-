...
async function testDirect() {
  console.log("Starting forced tool call test with model:", GEMINI_MODEL);
  ...
  try {
    session = await ai.live.connect({
      ...
      config: {
        ...
        systemInstruction: "You are a helpful calling agent. Always use check_room_availability tool.",
        tools: [{ functionDeclarations: hotelRoomTools }],
        toolConfig: {
          functionCallingConfig: {
            mode: "ANY" as any, // FORCE the model to call a tool
            allowedFunctionNames: ["check_room_availability"]
          }
        }
      },
      callbacks: {
        onmessage: async (msg) => {
          ...
          if (msg.serverContent) {
            console.log("[Direct] Server Content.");
          }
          if ((msg as any).toolCall?.functionCalls) {
            console.log("[Direct] Tool call detected! Executing...");
            try {
