...
export async function initiateOutboundCall(
  ...
  console.log(`[VoBiz Outbound] Answer URL: ${answerUrl}`);
  ...
  if (!response.ok) {
    console.error(`[VoBiz Outbound] API error:`, responseData);
    throw new Error(responseData?.error || responseData?.message || `VoBiz API returned ${response.status}`);
  }

  console.log(`[VoBiz Outbound] Call initiated successfully:`, responseData);
  ...
  // Auto-cleanup after 10 minutes (calls shouldn't last longer in normal usage)
  setTimeout(() => {
...
export async function hangupOutboundCall(callId: string): Promise<boolean> {
  ...
  try {
    ...
    console.log(`[VoBiz Outbound] Hangup response: ${response.status}`);
    updateCallStatus(callId, "completed");
    return true;
  } catch (err) {
    console.error(`[VoBiz Outbound] Hangup failed:`, err);
    return false;
  }
}
